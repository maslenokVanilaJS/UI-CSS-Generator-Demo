...
import'/src/js/magicBox'
 // Created with  MagicBox , handMade JavaScript library ....
let MB=MagicBox();

 //Return CSSText functions
function returnHover(options) {
  let CSSApliedArr=[];
  let cssInerText='';
       (options.tag.hover.hoverColor!=null)?CSSApliedArr.push(`background-color:${options.tag.hover.hoverColor};`):console.log('err');

   
       (options.tag.hover.hoverFilter!=null)?CSSApliedArr.push(`background-color:${options.tag.hover.hoverFilter};`):console.log('err');

   
 
for (const iterator of CSSApliedArr) {
  cssInerText+=`${iterator}\n`;
}
return cssInerText;
}


function returnActive(options) {
  let CSSApliedArr=[];
  let cssInerText='';
  (options.tag.active.activeColor!=null)?CSSApliedArr.push(`background-color:${options.tag.active.activeColor};`):console.log('err');
 (options.tag.active.activeFilter!=null)?CSSApliedArr.push(`background-color:${options.tag.active.activeFilter};`):console.log('err');
...
return cssInerText;
 ...
 function returnCSSAplied(options){
let CSSApliedArr=[];
let cssName=['width',
'height',
'box-shadow',
...
'filter'];
 ...
 (MB.s(options.tag).style.width!=``)?CSSApliedArr.push(`${cN[0]}:${MB.s(options.tag).style.width};`): console.log('error');   

 (MB.s(options.tag).style.height!=``)?CSSApliedArr.push(`${cN[1]}:${MB.s(options.tag).style.height};`): console.log('error');   

 (MB.s(options.tag).style.boxShadow!=``)?CSSApliedArr.push(`${cN[2]}:${MB.s(options.tag).style.boxShadow};`): console.log('error');   
 (MB.s(options.tag).style.backgroundColor!=``)?CSSApliedArr.push(`${cN[3]}:${MB.s(options.tag).style.backgroundColor};`): console.log('error');   
 (MB.s(options.tag).style.backgroundImage!=``)?CSSApliedArr.push(`${cN[4]}:${MB.s(options.tag).style.backgroundImage};`): console.log('error');   
 (MB.s(options.tag).style.borderRadius!=``)?CSSApliedArr.push(`${cN[5]}:${MB.s(options.tag).style.borderRadius};`): console.log('error');   
 (MB.s(options.tag).style.color!=``)?CSSApliedArr.push(`${cN[6]}:${MB.s(options.tag).style.color};`): console.log('error');   
 (MB.s(options.tag).style.fontSize!=``)?CSSApliedArr.push(`${cN[7]}:${MB.s(options.tag).style.fontSize};`): console.log('error');   
 (MB.s(options.tag).style.filter!=``)?CSSApliedArr.push(`${cN[8]}:${MB.s(options.tag).style.filter};`): console.log('error');   
...
// UI
   ...
   MB.s("#button").onclick=()=>{
    MB.hide("ResultView-ContentName");
    MB.hide_All("ResultView-Details");

     MB.hide("#img");
   MB.hide("#imageCSS");
   MB.hide('#codeResult');
   MB.hide("#getImgCode");
   MB.hide('#resetImgCode');
   MB.show("#buttonCSS");
    MB.show("#btn");
    MB.show("#getBtnCode");
    MB.show('#resetBtnCode');

   MB.s('#getBtnCode').onclick=()=>{
    
    MB.hide('#img');
    MB.hide('#btn');
    MB.s('#codeResult').innerText=`
<button id="btn">${MB.s('#btn').innerText}</button>
...
tag:doge
  ...
  MB.hide("#buttonCSS");
  MB.hide("#getBtnCode");
   MB.hide('#resetBtnCode');
  MB.show("#imageCSS");
    MB.show("#img");
    MB.show("#getImgCode");
    MB.show('#resetImgCode');

 
  MB.s('#getImgCode').onclick=()=>{
    MB.hide('#img');
    MB.hide('#btn');
    MB.s('#codeResult').innerText=`
<img id="image" src="yourSrc">
...
function activeConstructor(params) {
  ...
  this.BindValue=['#activeColorBind'];
...
//Bind
(this.filter.Target[i]==this.filter.Target[0])?MB.s(this.filter.Bind[i]).innerText=`sepia(${MB.s(this.filter.Target[i]).value}%)`:
(this.filter.Target[i]==this.filter.Target[1])?MB.s(this.filter.Bind[i]).innerText=`blur(${MB.s(this.filter.Target[i]).value}px)`:
(this.filter.Target[i]==this.filter.Target[2])?MB.s(this.filter.Bind[i]).innerText=`invert(${MB.s(this.filter.Target[i]).value}%)`:
(this.filter.Target[i]==this.filter.Target[3])?MB.s(this.filter.Bind[i]).innerText=`brightness(${MB.s(this.filter.Target[i]).value}%)`:
(this.filter.Target[i]==this.filter.Target[4])?MB.s(this.filter.Bind[i]).innerText=`opacity(${MB.s(this.filter.Target[i]).value}%)`:
(this.filter.Target[i]==this.filter.Target[5])?MB.s(this.filter.Bind[i]).innerText=`hue-rotate(${MB.s(this.filter.Target[i]).value}deg)`:
(this.filter.Target[i]==this.filter.Target[6])?MB.s(this.filter.Bind[i]).innerText=`grayscale(${MB.s(this.filter.Target[i]).value}%)`:
(this.filter.Target[i]==this.filter.Target[7])?MB.s(this.filter.Bind[i]).innerText=`contrast(${MB.s(this.filter.Target[i]).value}%)`:

MB.s(this.filter.Bind[i]).innerText=`saturate(${MB.s(this.filter.Target[i]).value}%)`;
...
this.activeApply= function x(params) {
   ...
   for (let i = 0; i <this.active.Target.length; i++) {
    ...
    MB.s(this.active.Target[i]).oninput=()=>{
     ...
     (this.active.Target[i]==this.active.Target[0])?MB.s(this.active.Bind[i]).innerText=`sepia(${MB.s(this.active.Target[i]).value}%)`:
     (this.active.Target[i]==this.active.Target[1])?MB.s(this.active.Bind[i]).innerText=`blur(${MB.s(this.active.Target[i]).value}px)`:
     (this.active.Target[i]==this.active.Target[2])?MB.s(this.active.Bind[i]).innerText=`invert(${MB.s(this.active.Target[i]).value}%)`:
     (this.active.Target[i]==this.active.Target[3])?MB.s(this.active.Bind[i]).innerText=`brightness(${MB.s(this.active.Target[i]).value}%)`:
     (this.active.Target[i]==this.active.Target[4])?MB.s(this.active.Bind[i]).innerText=`opacity(${MB.s(this.active.Target[i]).value}%)`:
     (this.active.Target[i]==this.active.Target[5])?MB.s(this.active.Bind[i]).innerText=`hue-rotate(${MB.s(this.active.Target[i]).value}deg)`:
     (this.active.Target[i]==this.active.Target[6])?MB.s(this.active.Bind[i]).innerText=`grayscale(${MB.s(this.active.Target[i]).value}%)`:
     (this.active.Target[i]==this.active.Target[7])?MB.s(this.active.Bind[i]).innerText=`contrast(${MB.s(this.active.Target[i]).value}%)`:
    
     MB.s(this.active.Bind[i]).innerText=`saturate(${MB.s(this.active.Target[i]).value}%)`;
...
this.hoverApply= function x(params) {
  ...
  for (let i = 0; i <this.hover.Target.length; i++) {
   ...
   MB.s(this.hover.Target[i]).oninput=()=>{
    ...
    (this.hover.Target[i]==this.hover.Target[0])?MB.s(this.hover.Bind[i]).innerText=`sepia(${MB.s(this.hover.Target[i]).value}%)`:
    (this.hover.Target[i]==this.hover.Target[1])?MB.s(this.hover.Bind[i]).innerText=`blur(${MB.s(this.hover.Target[i]).value}px)`:
    (this.hover.Target[i]==this.hover.Target[2])?MB.s(this.hover.Bind[i]).innerText=`invert(${MB.s(this.hover.Target[i]).value}%)`:
    (this.hover.Target[i]==this.hover.Target[3])?MB.s(this.hover.Bind[i]).innerText=`brightness(${MB.s(this.hover.Target[i]).value}%)`:
    (this.hover.Target[i]==this.hover.Target[4])?MB.s(this.hover.Bind[i]).innerText=`opacity(${MB.s(this.hover.Target[i]).value}%)`:
    (this.hover.Target[i]==this.hover.Target[5])?MB.s(this.hover.Bind[i]).innerText=`hue-rotate(${MB.s(this.hover.Target[i]).value}deg)`:
    (this.hover.Target[i]==this.hover.Target[6])?MB.s(this.hover.Bind[i]).innerText=`grayscale(${MB.s(this.hover.Target[i]).value}%)`:
    (this.hover.Target[i]==this.hover.Target[7])?MB.s(this.hover.Bind[i]).innerText=`contrast(${MB.s(this.hover.Target[i]).value}%)`:
   
    MB.s(this.hover.Bind[i]).innerText=`saturate(${MB.s(this.hover.Target[i]).value}%)`;
...
(this.radius.Target[i]==this.radius.Target[0])?MB.s(this.radius.BindValue[i]).innerText=`border-radius:${MB.s(this.radius.Target[i]).value}${this.radius.valuePx};`:
MB.s(this.radius.BindValue[i]).innerText=`border-radius:${MB.s(this.radius.Target[i]).value}${this.radius.valuePr};`;
};   
  }
};
}
...
(this.hover.Target[i]="#hoverColorTarget")?MB.s(this.hover.BindValue[i]).innerText=`${MB.s(this.hover.Target[i]).value}`:
MB.locFileBackground("#hoverBackImgTarget",'#btn');
...
(this.active.Target[i]="#activeColorTarget")?MB.s(this.active.BindValue[i]).innerText=`${MB.s(this.active.Target[i]).value}`:
MB.locFileBackground("#activeBackImgTarget",'#btn');
...
MB.s(this.background.BindValue[0]).innerText=`background-color:${MB.s(this.background.Target[0]).value};`
...
MB.s(this.background.BindValue[1]).innerText=`background-image:url('${MB.s(this.background.Target[1]).value}');`
...
(this.text.Target[i]=="#textValueTarget")?MB.s(this.text.BindValue[i]).innerText=`${MB.s(this.text.Target[i]).value}`:
(this.text.Target[i]=="#textColorTarget")?MB.s(this.text.BindValue[i]).innerText=`color:${MB.s(this.text.Target[i]).value};`:
(this.text.Target[i]=="#textFontStyleTarget")?MB.s(this.text.BindValue[i]).innerText=`${MB.s(this.text.Target[i]).value}`:
MB.s(this.text.BindValue[i]).innerText=`font-size:${MB.s(this.text.Target[i]).value}px`;
